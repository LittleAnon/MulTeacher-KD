...
ini_sections = 'finetune'
common_ini_sections = 'common'
task_to_keys = {
    "cola": ("sentence", None),
...
class DataTrainingArguments:
    ...
    the command line.
    """
    default_conf_name: Optional[str] = field(
        metadata={"help": "local config file name"},
    )
...
def main():
    ...
    ini_config = read_ini.read_properties_from_config_ini(data_args.default_conf_name)
    data_args.glue_datasets_local_dir = ini_config[ini_sections]['glue_datasets_local_dir']
    data_args.glue_metrics_local_dir = ini_config[ini_sections]['glue_metrics_local_dir']

    cache_dict = {
        'bert': [ini_config[ini_sections]['bert_base_model'], ini_config[common_ini_sections]['bert_finetune_model_path'].format(data_args.task_name)],
        'roberta': [ini_config[ini_sections]['roberta_base_model'], ini_config[common_ini_sections]['roberta_finetune_model_path'].format(data_args.task_name)],
        'gpt2': [ini_config[ini_sections]['gpt2_base_model'], ini_config[common_ini_sections]['gpt2_finetune_model_path'].format(data_args.task_name)]
    }
    # 为了不每回都传两长串路径，加了个这个
