...
TF_WEIGHTS_NAME = 'pytorch_model.ckpt'
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *inputs, **kwargs):
        ...
        if state_dict is None and not from_tf:
            ...
            if not os.path.exists(weights_path) and os.path.isdir(pretrained_model_name_or_path):
                ...
                latest_step = ""
                latest_step_point = 0
                for weights_path_tmp in weights_dir_files:
                    if 'pytorch_model.bin' in weights_path_tmp:
                        current_step = re.search('\d+', weights_path_tmp)
                        if current_step is not None:
                            current_step = int(current_step.group())
...
class TinyBertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids,  token_type_ids, attention_mask, hidden_out=True, attention_out=False, is_student = False):
