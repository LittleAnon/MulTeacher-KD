...
def loss_unit(emd, use_kd, use_emd, logits, y, teacher_logits, student_hidden, teacher_hidden):
    if self.use_kd:
        rep_loss = 0
        if self.use_emd:
            rep_loss = emd.loss(student_reps, teacher_hidden)
        kd_loss, _, _ = distillation_loss(logits, trn_ys, teacher_logits, self.net.output_mode,
                                          alpha=self.config.kd_alpha)
        loss = kd_loss + rep_loss * self.config.emd_rate
    else:
        loss = self.net.crit(logits, trn_ys)
